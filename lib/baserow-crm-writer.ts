...
export function createBaserowCrmWriter(options: {
  ...
  const { token, tables, fetcher = fetch, baseUrl = "https://api.baserow.io" } = options;
  ...
  return {
    async execute(command: LeadCommand, advisor: string): Promise<CrmData> {
      const before = await createConfiguredBaserowReader().load();
      ...
      await write(tables.leads, "PATCH", rowId(lead.id), leadPatch(lead));
      try {
      ...
      return createConfiguredBaserowReader().load();
    },
  };
}
