...
def col_index_to_letter(col_index):
    """Convierte un índice de columna 0-based a letra(s) de columna de Sheets (A, B, ..., Z, AA, ...)"""
    letter = ''
    col_index += 1  # 1-based
    while col_index > 0:
...
@retry(stop=stop_after_attempt(3), wait=wait_fixed(2), retry=retry_if_exception_type(Exception))
def update_cell(sheets_service, spreadsheet_id, tab_name, row_index, start_col_index, values_list):
    """Actualiza celdas usando índice de columna 0-based."""
    start_letter = col_index_to_letter(start_col_index)
    end_letter = col_index_to_letter(start_col_index + len(values_list) - 1)
    range_name = f"'{tab_name}'!{start_letter}{row_index}:{end_letter}{row_index}"
        ...
        if mode == "normal":
            if val_contacto == 'SÍ':
                ...
                if st.button("Marcar como Contactado", key=f"btn_{tab_name}_{lead['ROW_INDEX']}_{mode}"):
                    with st.spinner("Actualizando..."):
                        ...
                        update_cell(sheets_service, sheet_id, tab_name, lead['ROW_INDEX'], actual_sheet_idx, ["SÍ", 0, today_str])
            ...
            if st.button(f"Enviar Mensaje {drip_step + 1}", key=f"btn_drip_{tab_name}_{lead['ROW_INDEX']}_{drip_step}_{mode}"):
                with st.spinner("Actualizando Tracking..."):
                    today_str = datetime.now().strftime('%Y-%m-%d')
                    update_cell(sheets_service, sheet_id, tab_name, lead['ROW_INDEX'], actual_sheet_idx, ["SÍ", drip_step, today_str])
            ...
            if st.button("🏆 Venta Realizada", key=f"btn_venta_{tab_name}_{lead['ROW_INDEX']}_{mode}"):
                with st.spinner("Actualizando estado..."):
                    today_str = datetime.now().strftime('%Y-%m-%d')
                    update_cell(sheets_service, sheet_id, tab_name, lead['ROW_INDEX'], actual_sheet_idx, ["VENTA", drip_step, today_str])
            ...
            if st.button("❌ Lead Perdido", key=f"btn_perdido_{tab_name}_{lead['ROW_INDEX']}_{mode}"):
                with st.spinner("Actualizando estado..."):
                    today_str = datetime.now().strftime('%Y-%m-%d')
                    update_cell(sheets_service, sheet_id, tab_name, lead['ROW_INDEX'], actual_sheet_idx, ["PERDIDO", drip_step, today_str])
